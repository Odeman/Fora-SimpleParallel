...
//---------------------------------void for parallel------------------------------------------------
void parallel_query_task(Graph& graph, int query_size){
    int head;
    int source, temp;
    Fora_class fora_worker(graph);
...
void parallel_query(Graph& graph){
...
#pragma omp parallel for
        for(int i=0; i<OMP_num_threads; i++){
            parallel_query_task(graph, query_size);
        }
        printf("OMP_check_total_time: %.12f\n",omp_get_wtime()-OMP_check_time_start);
        /*
        for(int i=0; i<query_size; i++){
